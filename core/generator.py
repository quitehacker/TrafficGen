...
try:
    ...
    # telnetlib removed in Python 3.11+, skip telnet functionality
    telnetlib = None
import paramiko
try:
    import lib.xmpp as xmpp
except (ImportError, SyntaxError):
    # XMPP library has Python 2/3 compatibility issues
    xmpp = None
...
class telnet_gen():
    ...
    def __init__(self,
        ...
        if telnetlib is None:
            raise ImportError("telnetlib not available in Python 3.11+")
        self._host = params[0]
        self._port = params[1]
        self._user = params[2]
        self._pass = params[3]
    ...
    def __call__(self):
        if telnetlib is None:
            logging.getLogger(self.__generator__).error("telnetlib not available")
            return
...
class xmpp_gen():
    ...
    def __init__(self,
                 params):
        if xmpp is None:
            raise ImportError("XMPP library not available due to Python 3 compatibility issues")
        self._host = params[0]
        self._port = params[1]
        self._jid = params[2]
        self._pass = params[3]
    ...
    def __call__(self):
        if xmpp is None:
            logging.getLogger(self.__generator__).error("XMPP library not available")
            return
            ...
            if client.auth(jid.getNode(), self._pass, self._ressource):
                if client.isConnected():
                    ...
                    while datetime.datetime.now() < endtime:
                        ...
                        msg = ''.join(random.choice(string.ascii_letters) for _ in range(int(60 * random.random())))
